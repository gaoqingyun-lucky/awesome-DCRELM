...
def load_pretrain_parameter(model):
    ...
    pretrained_dict = torch.load('model_pretrain/{}_{}_pretrain.pkl'.format('ELM',opt.args.name), map_location='cpu')
...
def model_init(model, X, y, A_norm, init_center_index):
    """
    ...
    # model = test_pretrain(model,X,A_norm)
    # calculate embedding similarity
    with torch.no_grad():
        _, _, _, sim, _, _, _, Z, _, _ = model(X, A_norm, X, A_norm)

    # calculate cluster centers
    acc, nmi, ari, f1, centers = clustering(Z, y, init_center_index)

    return sim, centers

def ELM_model_init(X, y, init_center_index):
    """
    load the pre-train model and calculate similarity and cluster centers
    Args:
        model: Dual Correlation Reduction Network
    ...
    #     _, _, _, sim, _, _, _, Z, _, _ = model(X, A_norm, X, A_norm)
    sigmod = nn.Sigmoid()
    sim = sigmod(torch.mm(X, X.t()))#相似性矩阵sim：H*H的转置
    # calculate cluster centers
    acc, nmi, ari, f1, centers = clustering(X, y, init_center_index)#k-means聚类，centers是类簇中心
    print('ELM_KM_ACC: %f, ELM_KM_F1: %f, ELM_KM_ARI: %f, ELM_KM_NMI: %f' % (acc, f1, ari, nmi),end='\n')
...
def clustering(Z, y, init_center_index):
    """
    ...
    """
    model = KMeans(n_clusters=opt.args.n_clusters, init=Z.data.cpu().numpy()[init_center_index], n_init="auto")
...
def test_pretrain(model,X,A_norm):
    """
    train our model
    Args:
        model: Dual Correlation Reduction Network
    ...
    for epoch in tqdm.tqdm(range(50)):
        ...
        # input & output
        X_hat, Z_hat, A_hat, _, Z_ae_all, Z_gae_all, Q, Z, AZ_all, Z_all = model(X_tilde1, A_norm, X_tilde2, A_norm)
        # calculate loss:
